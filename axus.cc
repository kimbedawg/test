...
local function OnKeyPress(Input, GameProcessedEvent)
    if Input.KeyCode == getgenv().Axus.Aimbot.Keybind and not GameProcessedEvent then 
        Toggle = not Toggle
    elseif Input.KeyCode == getgenv().Axus.Macro.SpeedGlitchKey then
        if getgenv().Axus.Macro.Enabled then 
            getgenv().Axus.Macro.SpeedGlitch = not getgenv().Axus.Macro.SpeedGlitch
            if getgenv().Axus.Macro.SpeedGlitch then
                repeat
                    ...
                    keyrelease(0x4F)
                    game:GetService("RunService").Heartbeat:Wait()
                until not getgenv().Axus.Macro.SpeedGlitch
            end
        end
    end
end
...
UserInputService.InputBegan:Connect(function(keygo, ok)
    if (not ok) then
        if (keygo.KeyCode == getgenv().Axus.Aimbot.Keybind) then
            Locking = not Locking
            if Locking then
    ...
    if Player.Character and Player.Character:FindFirstChild("Humanoid") and Player.Character:FindFirstChild("Head") and Player.Character.Humanoid.Health ~= 0 and Player.Character:FindFirstChild("HumanoidRootPart") then
        for i, part in pairs(Player.Character:GetChildren()) do
            if part:IsA("BasePart") then
                ...
                local distToMouse = (Vector2.new(UserInputService:GetMouseLocation().X, UserInputService:GetMouseLocation().Y) - Vector2.new(screenPos.X, screenPos.Y)).Magnitude
                if distToMouse < closestDist and table.find(getgenv().Axus.Aimbot.MultipleTargetPart, part.Name) then
                    closestPart = part
                    closestDist = distToMouse
                end
            end
...
game:GetService("RunService").RenderStepped:Connect(function()
    if Plr and Plr.Character then
        if getgenv().Axus.Aimbot.NearestPart == true and getgenv().Axus.Aimbot.Basic == false then
            getgenv().Axus.Aimbot.TargetPart = tostring(getClosestPartToCursor(Plr))
        elseif getgenv().Axus.Aimbot.Basic == true and getgenv().Axus.Aimbot.NearestPart == false then
            getgenv().Axus.Aimbot.TargetPart = getgenv().Axus.Aimbot.TargetPart
        end
    end
end)
...
Client.Character.ChildAdded:Connect(function(child)
    if child:IsA("Tool") and child:FindFirstChild("MaxAmmo") then
        child.Activated:Connect(function()
            if Plr and Plr.Character then
                local Position = Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall and Plr.Character[getgenv().Axus.Aimbot.TargetPart].Position + Vector3.new(0, getgenv().Axus.Aimbot.JumpOffset, 0) or Plr.Character[getgenv().Axus.Aimbot.TargetPart].Position
                if game.PlaceId == 2788229376 or game.PlaceId == 7213786345 or game.PlaceId == 16033173781 or game.PlaceId == 16158576873 then 
                    mainevent:FireServer("UpdateMousePosI", Position + ((Plr.Character.HumanoidRootPart.Velocity) * getgenv().Axus.Aimbot.Prediction))
                else
                    mainevent:FireServer("UpdateMousePos", Position + ((Plr.Character.Humanoid.MoveDirection * Plr.Character.Humanoid.WalkSpeed) * getgenv().Axus.Aimbot.Prediction))
                end
            end
        end)
    end
...
game:GetService("RunService").RenderStepped:Connect(function()
    if Plr ~= nil and Plr.Character then
        ...
        if not CheckAnti(Plr) then
            local Main = CFrame.new(workspace.CurrentCamera.CFrame.p, Position + ((Plr.Character.HumanoidRootPart.Velocity) * getgenv().Axus.Aimbot.AimBotSkid) + GetShakedVector3(getgenv().Axus.Aimbot.Shake))
            workspace.CurrentCamera.CFrame = workspace.CurrentCamera.CFrame:Lerp(Main, getgenv().Axus.Aimbot.Amount, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
        else
            local Main = CFrame.new(workspace.CurrentCamera.CFrame.p, Position + ((Plr.Character.Humanoid.MoveDirection * Plr.Character.Humanoid.WalkSpeed) * getgenv().Axus.Aimbot.AimBotSkid) + GetShakedVector3(getgenv().Axus.Aimbot.CameraShake))
            workspace.CurrentCamera.CFrame = workspace.CurrentCamera.CFrame:Lerp(Main, getgenv().Axus.Aimbot.Amount, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
        end
    end
    if getgenv().Axus.Checks.PlayerDeath == true and Plr and Plr.Character then
    ...
    if getgenv().Axus.Checks.TargetDeath == true and Plr and Plr.Character:FindFirstChild("Humanoid") then
    ...
    if getgenv().Axus.Checks.PlayerDeath  == true and Client.Character and Client.Character:FindFirstChild("Humanoid") and Client.Character.Humanoid.health < 1 then
        if Locking == true then
            Plr = nil
            Locking = false
        end
    end
    if getgenv().Axus.Safety.AntiGroundShots == true and Plr.Character.Humanoid.Jump == true and Plr.Character.Humanoid.FloorMaterial == Enum.Material.Air then
        pcall(function()
...
if getgenv().Axus.Spin.Enabled == true then
    
    local Players = game:GetService("Players")
    local UserInputService = game:GetService("UserInputService")
    local RunService = game:GetService("RunService")
    local Camera = workspace.CurrentCamera
    local Toggle = getgenv().Axus.Spin.Enabled
    local RotationSpeed = getgenv().Axus.Spin.SpinSpeed
    local Keybind = getgenv().Axus.Spin.Keybind
    ...
    local function RotateCamera()
        if Toggle then
            ...
            if TotalRotation >= getgenv().Axus.Spin.Degrees then 
                Toggle = false
                TotalRotation = 0
            end
        end
